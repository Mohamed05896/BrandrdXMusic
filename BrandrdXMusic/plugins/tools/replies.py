...
SAD_QUOTES = [
    ...
    "الـخـيـبـة هـي أن تـغـفـو وأنـت مـمـتـلـئ بـكـلـمـات لـم تـجـد مـن يـسـمـعـهـا 🖤🥀", "كـنـا نـحـتـاج فـقـط لـشـخـص يـࢪى خـلـف صـمـتـنـا 🖤🥀",
...
# --- [ 1. نـظـام الـࢪدود الـشـامـل (مـيـديـا + مـتـعـدد) ] ---

@app.on_message(filters.command(["اضف رد", "اضف ردي", "اضف رد متعدد"], "") & filters.group)
async def start_add_boda(_, m: Message):
    if "ردي" not in m.text and not await is_admin(m.chat.id, m.from_user.id):
        return await m.reply_text("• عـمـࢪي هـذا لـلـمـشـࢪفـيـن بـس")
    
    mode = "multi" if "متعدد" in m.text else "user" if "ردي" in m.text else "general"
    user_state[m.from_user.id] = {"step": "word", "mode": mode, "chat_id": m.chat.id}
    await m.reply_text("• اࢪسـل الـكـلـمـه الآن")

@app.on_message(filters.group & ~filters.bot, group=1)
async def handle_db_input(client, m: Message):
    uid = m.from_user.id
    if uid not in user_state: return
    state = user_state[uid]
    
    if state["step"] == "word":
        if not m.text: return await m.reply_text("• اࢪسـل نـص يـا عـمـࢪي")
        user_state[uid].update({"step": "res", "word": m.text})
        await m.reply_text("• اࢪسـل الـجـواب (نص، صورة، ملصق، بصمة)")
    
    elif state["step"] == "res":
        word, mode = state["word"], state["mode"]
        res_data = {"word": word, "user_id": uid}
        
        if m.text: res_data.update({"type": "text", "file": m.text})
        elif m.photo: res_data.update({"type": "photo", "file": m.photo.file_id, "cap": m.caption})
        elif m.sticker: res_data.update({"type": "sticker", "file": m.sticker.file_id})
        elif m.voice: res_data.update({"type": "voice", "file": m.voice.file_id})
        
        if mode == "multi":
            await db.update_one({"chat_id": m.chat.id, "word": word}, {"$push": {"responses": res_data}}, upsert=True)
        else:
            await db.update_one({"chat_id": m.chat.id, "word": word}, {"$set": res_data}, upsert=True)
        
        del user_state[uid]
        await m.reply_text(f"• تـم حـفـظ 「 {word} 」 بـنـجـاح")

# --- [ 2. نـظـام الـهـمـسـات والـزاجـل ] ---
...
async def open_whisper(_, q: CallbackQuery):
    ...
    await q.answer(f"الـࢪسـالـه :\n\n{data['msg']}", show_alert=True)

# --- [ 3. أوامـࢪ الـمـسـح والـتـنـفـيـذ ] ---

@app.on_message(filters.command(["مسح رد", "مسح الردود"], "") & filters.group)
async def delete_rep(_, m: Message):
    if not await is_admin(m.chat.id, m.from_user.id): return
    if "الردود" in m.text:
        await db.delete_many({"chat_id": m.chat.id})
        return await m.reply_text("• تـم مـسـح كـل الـࢪدود")
    word = m.text.split(None, 1)[1] if len(m.command) > 1 else None
    if word:
        await db.delete_one({"chat_id": m.chat.id, "word": word})
        await m.reply_text(f"• تـم مـسـح الـࢪد 「 {word} 」")
