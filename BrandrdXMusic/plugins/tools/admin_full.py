...
# [ الـحـالـة: مـطـابـق لـشـروط الايمـوجـي 100% ]
# ==================================================================================================
...
async def rank_logic(client: Client, message: Message):
    ...
    if text == "رفع مشرف":
        ...
        try:
            await message.chat.promote_member(target.id, privileges=ChatPrivileges(
                can_manage_chat=True, can_delete_messages=True, can_manage_video_chats=True,
                can_restrict_members=True, can_promote_members=True, can_change_info=True,
                can_invite_users=True, can_pin_messages=True
            ))
            await message.reply_text(f"🤍 ¦ تـم رفـعـه مـشـرف بـكـل الـصـلاحـيـات.")
        except: await message.reply_text("🥀 ¦ لـيـس لـدي صـلاحـيـة.")
        return
...
LOCKS_MAP = {
    "امر حظر": "ban", "امر كتم": "mute", "امر رفع": "promote",
...
async def wipe_logic(client: Client, message: Message):
    ...
    if target == "امسح" or target == "الميديا":
        ids = [m.id async for m in message.chat.get_history(limit=50) if m.media]
        if ids: await app.delete_messages(cid, ids)
        return await message.reply_text(f"🧹 ¦ تـم تـنـظـيـف {len(ids)} رسـالـة.")

# ==================================================================================================
# [ 7 ] الـعـقـوبـات (Actions)
# ==================================================================================================

@app.on_message(filters.command(["حظر", "كتم", "طرد", "تقييد", "الغاء حظر", "الغاء كتم", "الغاء تقييد", "رفع القيود", "طرد المحذوفين", "طرد البوتات", "كشف البوتات"], "") & filters.group)
async def actions_logic(client: Client, message: Message):
    cmd = message.command[0]
    cid = message.chat.id
    uid = message.from_user.id

    lock_key = "mute" if cmd in ["كتم", "الغاء كتم"] else "ban"
    ...
    try:
        if cmd == "حظر":
            ...
            await message.reply_text(f"🥀 ¦ تـم كـتـم {target.mention}.")
        elif cmd == "الغاء كتم":
            await message.chat.restrict_member(target.id, ChatPermissions(can_send_messages=True))
            await message.reply_text(f"♥️ ¦ تـم الـغـاء الـكـتـم.")
    except: await message.reply_text("🤎 ¦ خـطـأ فـي الـتـنـفـيـذ.")

# ==================================================================================================
# [ 8 ] لـوحـة الـتـحـكـم (Dashboard) - تـم تـعـديـل الايمـوجـي
# ==================================================================================================
...
async def open_dashboard(client: Client, message: Message):
    ...
    # تـم اسـتـبـدال كـل الايمـوجـي الـخـارجـيـة هـنـا
