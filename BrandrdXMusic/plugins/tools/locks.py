...
@app.on_message(filters.command(["كتم", "ميوت"], "") & filters.group)
...
# --- [ 4. لوحة الإعدادات بالزخرفة المطلوبة ] ---

def get_kb(chat_id):
    kb, active = [], smart_db.get(chat_id, set())
    unique_keys = list(dict.fromkeys(LOCK_MAP.values()))
    names_map = {v: k for k, v in LOCK_MAP.items()}
    
    for i in range(0, len(unique_keys), 2):
        row = []
        k1 = unique_keys[i]
        n1 = names_map[k1]
    ...
    kb.append([InlineKeyboardButton("• المجموعة ⇽ مـفـتـوح •", callback_data="none")])
...
async def cb_handler(_, cb: CallbackQuery):
    ...
    if cb.data.startswith("u_unmute_"):
        if not await has_permission(cb.message.chat.id, cb.from_user.id):
            return await cb.answer("هـذا الأمـر لـلـمـشـرفـين فـقـط", show_alert=True)
...
async def protector_engine(_, message: Message):
    ...
    if "porn" in locks and text:
        ...
        if any(fuzz.ratio(bad, word) > 85 for word in clean.split() for bad in BAD_WORDS):
            ...
            await message.reply(f"<b>• يـا {message.from_user.mention}، تـذكـر قـول الله تـعـالـي: (مَا يَلْفِظُ مِنْ قَوْلٍ إِلَّا لَدَيْهِ رَقِيبٌ عَتِيدٌ) 🤍🥀</b>")
    ...
    check = [
        ...
        ("slashes", "/" in text or "\\" in text), ("hashtags", "#" in text)
    ]
...
async def clear_chat_cmd(_, message: Message):
    ...
    t = await message.reply("<b>• تـم الـتـنـظـيـف بـنـجـاح •</b>")
