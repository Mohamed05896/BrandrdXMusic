...
async def get_locks(chat_id):
    # جلب وثيقة الأقفال الخاصة بالمجموعة
...
async def update_lock(chat_id, key, lock=True):
    # تحديث حالة القفل (إضافة أو إزالة من القائمة)
    if lock:
...
async def get_warn_limit(chat_id):
    # جلب الحد الأقصى للتحذيرات (الافتراضي 3)
...
async def get_current_warns(chat_id, user_id):
    # معرفة عدد تحذيرات العضو الحالية
    doc = await db_warns.find_one({"chat_id": chat_id})
...
async def has_permission(chat_id, user_id):
    # التحقق مما إذا كان المستخدم مطوراً أو مشرفاً
...
async def force_delete(chat_id, current_id, limit):
    # حذف الرسائل القديمة (التنظيف) بدفعات
    count = 0
...
def check_porn_api(file_path):
    # فحص الصور عبر API (يتم تشغيلها في Thread منفصل)
    try:
...
async def admin_cmds_handler(_, message: Message):
    ...
    # تنفيذ الأوامر (نفس النصوص القديمة)
    try:
        if cmd == "سماح":
            ...
            await message.reply(f"<b>• تم منح السماح لـ {mention}</b>")
        elif cmd == "شد سماح":
        ...
        # --- تم تغيير الشرط هنا أيضاً ---
            ...
            await message.reply(f"<b>• تم فك كتم {mention}</b>")
    except: pass
...
async def set_warns_cmd(_, message: Message):
    ...
    try:
        ...
        await message.reply(f"<b>• تم تحديد التحذيرات بـ {num}</b>")
    except: pass
...
async def destructive_clear(_, message: Message):
    ...
    temp = await message.reply(f"<b>• تم مسح {deleted} رسالة</b>")  
...
async def self_destruct(_, message: Message):
    ...
    await message.reply("<b>اضغط للبدء في تدمير آخر 500 رسالة</b>", reply_markup=kb)
...
async def toggle_lock(_, message: Message):
    ...
    if cmd == "قفل":
        ...
        await message.reply(f"<b>• تم قفل ({input_text})</b>")
    else:
        await update_lock(message.chat.id, key, False)
        await message.reply(f"<b>• تم فتح ({input_text})</b>")
...
async def callback(_, cb: CallbackQuery):
    ...
    if cb.data == "total_destruction":  
        ...
        await cb.message.edit("<b>جاري التدمير...</b>")  
        deleted = await force_delete(cb.message.chat.id, cb.message.id, 500)  
        await app.send_message(cb.message.chat.id, f"<b>تم تدمير {deleted} رسالة</b>")  
